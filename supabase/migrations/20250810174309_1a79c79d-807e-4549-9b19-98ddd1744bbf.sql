...
CREATE TABLE public.job_posts (
  ...
  search_tsv tsvector GENERATED ALWAYS AS (
    ...
    setweight(to_tsvector('french', commune), 'B') ||
    setweight(to_tsvector('french', description), 'C') ||
    setweight(to_tsvector('french', array_to_string(skills, ' ')), 'D')
  ) STORED
);
...
CREATE INDEX idx_job_posts_embedding ON public.job_posts USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);
